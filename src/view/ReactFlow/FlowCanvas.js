...
import { setRuleChainNode } from '../../store/flowReducer'
// 自定义节点
const nodeTypes = {
  action: RelationNode,
  input: RelationNode,
...
export default function FlowCanvas() {
  ...
  const onConnect = (params) => {
    ...
    setEdges((eds) =>
      (eds || []).concat({
        ...
        label: 'Edge Label',
        // type: isInput ? 'input' : 'link', // 这里的类型和edgeTypes 里面对应的类型有关，这里如果是input， 应该要设置一个input 的类型
        type: 'link',
        ...params
      })
    );
  ...
  const onDrop = (event) => {
    ...
    // 获取更新前的节点数组
    const existingNodes = flowStoreState.nodes;
    const newNodes = [...existingNodes, newNode]; // 构建新的节点数组

    dispatch(setRuleChainNode(
        { data: newNodes }
    )); // 分发action，更新节点数组
